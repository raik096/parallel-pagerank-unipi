...
#define Buf_size 10 // la dimensione del buffer circolare

typedef struct {        // na struttura del grafo condiviso
    int N;              // numero di nodi che appartengono al grafo letto
    int n_archi;        // numero di archi, desunto dal file mtx
    int n_archi_validi; // numero di archi validi, ovvero non ridondanti né doppi
    int pilaindici;     // utilizzata per smistare il lavoro tra i thread dinamicamente

    int *out;           // array di interi che descrive il numero di nodi in uscita dalla cella out[i]
    inmap *in;          // array di puntatori a struttura inmap, Inizializzato come puntatore a NULL

    double *x;          // vettore contenente il PageRank all'iterazioné "t" dove ogni cella coincide con un nodo, cioè pagina
    double *xnext;      // come sopra ma all'iterazioné "t+1"
    double d;           // valore di dumping utile all'algoritmo di pageranking
    double s_t;         // valore s_t utile all'algoritmo PK
    double k;           //  " "
    double scarto;      //  " "
    int taux;           // # thread ausiliari
    ...
    pthread_cond_t *down; // ptr di una cond. variaible che va dal main verso i taux
    pthread_cond_t *up;   // ptr di una cond. var. che va dai taux al main
    pthread_mutex_t *mu3;
} grafo;


typedef struct {     //la struct dei dati in supporto ai thread
    ...
    sem_t *sem_free_slots;      // ptr al semaforo condiviso tra thread cons e prod
    sem_t *sem_data_items;      //  " "
    pthread_mutex_t *mu;         // ptr al mutex condiviso tra i thread
    pthread_mutex_t *mu2;        // ptr al secondo mutex condiviso tra i thread
    pthread_barrier_t *barriera;

    int startindex;             // ogni thread inizia da un pt specifico
    int endindex;               // ogni thread finisce ad un pt specifico
    int *size_in;               // l'allocazioné dinamica per in condivisa tra i thread
} dati;

typedef struct {

    bool *apparecchiato;         // necessario per capire se e' iniziata la fase di calcolo
    bool *procedi;               // booleano per fermare il programma
} dati_h;
...
{
    int opt;                // var in support alla funzioné getopt
    ...
    grafo *g = malloc(sizeof(grafo)); // alloco una porzioné di memoria per il grafo
    if (g == NULL) {
    ...
    pthread_t t[taux]; // creazioné statica dell'array contente i thread, ad ogni cella avro' un tid
    pthread_t t_h;     // creazioné thread gestore dei segnali

    sem_t sem_free_slots, sem_data_items;
    pthread_barrier_t barriera;

    pthread_cond_t up = PTHREAD_COND_INITIALIZER;    // la variabile che serve ai taux per svegliare il main thread
    pthread_cond_t down = PTHREAD_COND_INITIALIZER;  // la variaible che serve al main per fare il broadcast sui thread
    ...
    pthread_barrier_init(&barriera, NULL, taux);                 // creo l'unica barriera per coordinare l'algoritmo di PRK
    ...
    int datiG[3];       // datiG[0]: #RIGHE, in 1:#COLONNE, in 2:#ARCHI
    size_t size = 0;
    char *linea = NULL; // in modo che la getliné allochi la memoria autonomamente
    char *token = NULL;

    // ciclo necessario a scorrere le righe non significative
    ...
    int size_in  = 10;         // parto con una dimensione bassa sull'allocazioné della memoria dedicata a g->in
    g->out       = calloc(g->N, sizeof(int));
    g->in        = malloc(sizeof(inmap) * size_in);
    g->pilaindici = (g->N)-1; // necessario per la suddivisioné di carico dinamica tra i thread
    g->mu3       = &mu3;
    g->up        = &up;
    g->down      = &down;
    g->x         = NULL;
    ...
    linea = NULL;              // per scongiurare possibili errori di getliné alla prossima chiamata
    ...
    sigset_t mask;
    sigfillset(&mask); // riempio l'insieme mask con tutti i segnali
    sigdelset(&mask, SIGINT);
    pthread_sigmask(SIG_BLOCK, &mask, NULL); // e lo blocco, i taux erediteranno il blocco

    // creazione del thread handler
    dati_h dh;
    dh.apparecchiato    = &g->apparecchiato;
    dh.procedi          = &g->procedi;

    xpthread_create(&t_h, NULL, &handler, &dh, __LINE__, __FILE__);     


    double chunk = floor((double)g->N/(double)taux); // suddivido il carico tra taux
    for (int i = 0; i < taux; i++) {                   // creazioné sottothread
        ...
        a[i].ptrG           = g;
        a[i].buffer         = buffer;                  // il buffer verra' popolato da puntatori a carattere, restituiti dal thread main con la funzioné strdup
        a[i].cindex         = &cindex;                 // tutti i sotto thread fanno riferimento ad un indice consumatore
        a[i].size_in        = &size_in;
        a[i].mu             = &mu;
        a[i].mu2            = &mu2;
        a[i].barriera       = &barriera;
        a[i].sem_data_items = &sem_data_items;          // il primo semaforo condiviso tra tutti i thread
        a[i].sem_free_slots = &sem_free_slots;          // il secondo semaforo condiviso tra tutti i thread
    ...
    for (int i = 0; i < taux; i++) {                        // inserisco valore di terminazioné ai consumatori
    ...
    // si e' verificato il segnale
    if (!g->procedi) {
        
        int indice_max = 0;
        double val_max = 0.0;
        for (int i = 0; i < g->N; i++) {
            if (val_max < x[i]) val_max = x[i];
            indice_max = i;
        }

        fprintf(stderr, "Stop by SIGUSR1\n at iteration: %d\n actual top page: %d\n with value: %f\n", numiter, indice_max, val_max);
        free(g->out);
        free(g->x);
    ...
    // non si e' verificato il segnale
    if (g->procedi) {
        ...
        // cmp e' nel file liste.c
        qsort(ptr_x, (g->N), sizeof(double *), cmp);
        ...
        // uso l'aritmetica dei puntatori per non perdere il riferimento all'indice cioe' al nodo
        while (shownodes != k)
        {
            fprintf(stdout, " %ld %f\n", (ptr_x[k]-x), *ptr_x[k]);
            k++;
    ...
    pthread_kill(t_h, SIGUSR2);
    xpthread_join(t_h, NULL, __LINE__, __FILE__);
...
void *tbody(void *arg)
{
    ...
    while(true)
    {
        ...
        *(a->cindex) += 1;                                // il thread aggiorna la variabile
        ...
        token = strtok_r(NULL, " ", &var_r);          // ottiené il secondo pezzo della stringa
        ...
        if (temp[0] != temp[1]) { // evita l'arco rindondante
            ...
            // se l'indice di IN è maggiore della dimensioné attuale, rialloca la memoria
            if (temp[1] >= *a->size_in) {
    ...
    // posso calcolare il primo fattore di x^(t+1) una sola volta, distribuisco il carico tra i thread
    double val = 1.0/(double)g->N; // prima iterazione, inizializzazioné del vettore x
    ...
    while (continua)
    {
        ...
        // all'alba della nuova iterazioné c'e' bisogno che il vettore xnext sia diventato x
        ...
        while (true)
        {   
            // non piu' distribuzioné di carico ma meccanismo a pila di indici dinamico a consumo
            xpthread_mutex_lock(a->mu, __LINE__, __FILE__);
            int j = g->pilaindici; // il thread si prende in carico l'indice j
            if (j == -1) {         // sono arrivato alla finé dei nodi da calcolare, mi metto in attesa dei check della funzione
                xpthread_mutex_unlock(a->mu, __LINE__, __FILE__);
                break;
            }
...
{   
    ...
    int s;
    do {
        int e = sigwait(&mask, &s);  // aspetta finche' non arriva un segnale che gestisce
        if (e != 0) perror("Errore sigwait");
        if (s == SIGUSR1) {
            if ( *d->apparecchiato == true) {
                *d->procedi = false;
                break;
            } else {
                fprintf(stderr, "Calcolo PageRank non ancora iniziato: SIGUSR1 inefficace\n");
            }
        }
